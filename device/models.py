...
#使用函数 ugettext_lazy() 来指定一个翻译字符串。作为惯例，使用短别名 _ 来引入这个函数以节省键入时间.
...
class CtrlChannel(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)  #是uuid生成的随机的长字符串
...
class MetricData(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if queryset:
            ...
            if((self.value == tmp.value) and (self.direction =='UP')):  #如果最新上传数据和最后数据相同，没更新，就不保存
                pass
            else:
