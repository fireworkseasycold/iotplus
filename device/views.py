...
class DeviceViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request,):
        # print(request.META)
        queryset = Device.objects.filter(owner=request.user.id).order_by('-created')
        #上面这句在swaggerui里配置会jwt认证失败导致报错;但是使用postman带bearertoken在访问发现接口没有错，
        # 最终解决方法发现request.user不行，使用request.user.id解决了，https://stackoverflow.com/questions/62966136/typeerror-field-id-expected-a-number-but-got-django-contrib-auth-models-anon
        #request.user不行原因是simplejwt导致：https://blog.csdn.net/lady_killer9/article/details/103075076
        #不算解决跳过 #不使用token的认证访问会出错，测试用时用下方的try,except取代上一句来规避登录认证,swaggertoken输入authrrize登录认证后则不用
        # try:
        #     queryset = Device.objects.filter(owner=request.user).order_by('-created')  #只搜索到自己创建的
        # except:
        ...
        #         queryset=queryset
        #     else:
        #         queryset = self.queryset  # 添加个登录判断没登陆则显示所有（没有则为【】）,当前测试用，直接用62行的会没登陆时候测试http://localhost:8000/device/devices/时候报错
        self.check_object_permissions(request,Device)
        serializer = DeviceSerializer(queryset, many=True)
        return Response(serializer.data)

    def retrieve(self, request, pk=None):
        queryset = Device.objects.filter(owner=request.user.id)
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            try:
                if request.data["direction"] == "DOWN":
                    ctrlchannel_id = request.data["ctrlchannel"]
                    payload = json.dumps(request.data)
                    publish.single(ctrlchannel_id,payload,hostname=settings.MQTT_HOST)
            except:
                pass
